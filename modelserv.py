...
#     log.info("Client %s connected to GPT-4 endpoint", client_host)
#     time_start = datetime.datetime.now()
#     try:
#         msg = await ws.receive_json()
...
#         if not messages:
#             await manager.send_json(ws, {"error": "Missing 'messages' in payload"})
#             return
...
#                     if delta.content:
#                         await manager.send_json(ws, {"token": delta.content})

#                 await manager.send_json(ws, {"token": None, "flag": True})

#             except Exception as e:
#                 log.exception("GPT-4 streaming error for client %s: %s", client_host, e)
#                 await manager.send_json(ws, {"error": f"GPT-4 streaming failed: {str(e)}"})
...
#                 await manager.send_json(ws, {"error": f"GPT-4 request failed: {str(e)}"})
...
#         try:
#             await manager.send_json(ws, {"error": f"Server error: {str(exc)}"})
#         except:
#             pass
#     finally:
#         manager.disconnect(ws)
...
async def websocket_endpoint_kimi(ws: WebSocket):
    await manager.connect(ws)
    client_host = ws.client.host
    log.info("Client %s connected into kimi endpoint for generation", client_host)
    time_start = datetime.datetime.now()
    try:
        msg = await ws.receive_json()
        model = msg.get("model", "moonshotai/kimi-k2-instruct-0905")
        messages = msg.get("messages")
        stream = msg.get("stream", True)
        ...
        if stream:
            try:
                ...
                await manager.send_json(ws, {"error": f"Kimi streaming failed: {str(e)}"})
            ...
            try:
                ...
                await manager.send_json(ws, {"error": f"Kimi request failed: {str(e)}"})
